...
M = 3000
density = 850
tank = [[8.91304348, 1.20652174, 0.61669004, 1.5, 0.9, 0.3, 0.2098],
        [6.91304348, -1.39347826, 0.21669004, 2.2, 0.8, 1.1, 1.8531],
        [-1.68695652, 1.20652174, -0.28330996, 2.4, 1.1, 0.9, 2.376],
        [3.11304348, 0.60652174, -0.18330996, 1.7, 1.3, 1.2, 2.652],
        [-5.28695652, -0.29347826, 0.41669004, 2.4, 1.2, 1, 2.4131],
        [-2.08695652, -1.49347826, 0.21669004, 2.4, 1, 0.5, 1.2]]
limit = (1.1, 1.8, 1.7, 1.5, 1.6, 1.1)
t2t = {0: 1, 5: 4}


# tank = [[8.91304348, 1.20652174, 0.61669004, 1.5, 0.9, 0.3, 0.1561],
#         [6.91304348, -1.39347826, 0.21669004, 2.2, 0.8, 1.1, 1.8297],
#         [-1.68695652, 1.20652174, -0.28330996, 2.4, 1.1, 0.9, 2.376],
#         [3.11304348, 0.60652174, -0.18330996, 1.7, 1.3, 1.2, 2.652],
#         [-5.28695652, -0.29347826, 0.41669004, 2.4, 1.2, 1, 2.3012],
#         [-2.08695652, -1.49347826, 0.21669004, 2.4, 1, 0.5, 1.2]]
...
if __name__ == '__main__':
    ...
    V = np.zeros([time, 6])
    cur = centroid(tank)
    start_v = sum(np.array(tank)[:, 6])
    print(sum(cost_list) / 850, start_v)
    ...
    for i in range(6):
        plot(V[:, i], limit[i], i)
        print(i, sum(V[:, i]))
    print(sum(np.array(tank)[:, 6]), start_v - sum(np.array(tank)[:, 6]))
