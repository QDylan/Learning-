# -*- coding: utf-8 -*-
"""
 @Time    : 2020-10-16 10:38
 @Author  : QDY
 @FileName: 457. 环形数组循环.py
 @Software: PyCharm
"""
"""
给定一个含有正整数和负整数的环形数组nums。 如果某个索引中的数 k为正数，则向前移动 k个索引。
相反，如果是负数 (-k)，则向后移动 k个索引。因为数组是环形的，所以可以假设最后一个元素的下一个元素是第一个元素，而第一个元素的前一个元素是最后一个元素。

确定 nums中是否存在循环（或周期）。循环必须在相同的索引处开始和结束并且循环长度 > 1。
...
class Solution:
    def circularArrayLoop(self, nums) -> bool:
        ...
        for i in range(N):  # 遍历每个节点（作为起点）
            if nums[i] == 0: continue  # 遇到访问过的点，跳过
            slow, fast = i, nxt(i)
            # 遇到相反符号的节点 就退出
