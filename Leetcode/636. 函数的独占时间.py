# -*- coding: utf-8 -*-
"""
 @Time    : 2020-10-02 17:36
 @Author  : QDY
 @FileName: 636. 函数的独占时间.py
 @Software: PyCharm
"""
"""
...
class Solution:
    def exclusiveTime(self, n: int, logs):
        stack, res, interval = [], [0] * n, 0
        for l in logs:
            index, status, time = l.split(':')
            index = int(index)
            time = int(time)
            if status == 'start':  # 遇start入栈
                stack.append((index, time))
            else:
                interval = time - stack.pop()[1] + 1
                res[index] += interval
                if stack:  # 遇end出栈，下一个出栈的要减去内部函数占用的时间
