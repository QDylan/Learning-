# -*- coding: utf-8 -*-
"""
 @Time    : 2020-10-05 11:01
 @Author  : QDY
 @FileName: 1610. 可见点的最大数目.py
 @Software: PyCharm
"""
"""
    ...
    def visiblePoints(self, points, angle: int, location) -> int:
        ...
        for x, y in points:
            if x == location[0] and y == location[1]:
                same += 1  # 与location相同的点
                continue
            tmp = degrees(atan2(y - location[1], x - location[0]))
            d.append(tmp)
            d.append(tmp + 360)  # 构成一个环
        d.sort()
        start = 0
        for i in range(len(d)):
            if d[i] - d[start] <= angle:
                res = max(res, i - start + 1)
            else:
                while d[i] - d[start] > angle:
                    start += 1
        return res + same
