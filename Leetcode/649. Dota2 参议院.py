# -*- coding: utf-8 -*-
"""
 @Time    : 2020-10-10 11:06
 @Author  : QDY
 @FileName: 649. Dota2 参议院.py
 @Software: PyCharm
"""
"""
...
class Solution:
    def predictPartyVictory(self, senate: str) -> str:
        q = deque([])  # 双端队列
        people, bans = [0, 0], [0, 0]  # 0号位表示Radiant,1号位表示Dire
        h = {'R': 0, 'D': 1}
        ...
        while all(people):  # 只剩下一种阵营时跳出循环
            x = q.popleft()  # 从前往后取出
            if bans[x]:  # 若 x阵营被ban了
                bans[x] -= 1  # 消耗一个ban
                people[x] -= 1  # x阵营存活人数-1
            else:  # x阵营没被ban,
                bans[x ^ 1] += 1  # 可以去ban对方阵营
                q.append(x)  # 并存活下来
