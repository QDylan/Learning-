# -*- coding: utf-8 -*-
"""
 @Time    : 2020-11-05 10:41
 @Author  : QDY
 @FileName: 1639. 通过给定词典构造目标字符串的方案数.py
 @Software: PyCharm
"""
"""
...
一旦你使用了 words中第 j个字符串的第 k个字符，你不能再使用 words字符串列表中任意单词的第 x个字符（x <= k）。
也就是说，所有单词下标小于等于 k的字符都不能再被使用。
...
class Solution:
    def numWays(self, words, target: str) -> int:
        mod, len_w, len_t = 10 ** 9 + 7, len(words[0]), len(target)

        cnts = [[0] * 26 for i in range(len_w)]  # cnts[i][c] 为 字母c在所有word的第i个位置出现的次数
        for w in words:
            for i in range(len_w):
                cnts[i][ord(w[i]) - 97] += 1

        @lru_cache(None)
        def dp(i, j):  # 从word[i:]开始，构造target[j:]的方案数
            if i >= len_t: return 1
            if j >= len_w: return 0
            return (dp(i, j + 1) + cnts[j][ord(target[i]) - 97] * dp(i + 1, j + 1)) % mod
